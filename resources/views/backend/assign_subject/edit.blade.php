...
<div class="container-fluid">
    <div class="row justify-content-center">
      <div class="col-12 col-lg-10 col-xl-8">
        ...
        <div class="my-4">
          <form method="POST" action="{{route('setup.assign_subject.update', $data[0]->class_id)}}">
            @csrf
            <hr class="my-4">
            <div class="add_item">
              <div class="form-group ">
                ...
                <select id="inputState5" name="class_id" class="form-control @error ('class_id') is-invalid @enderror">
                  ...
                  <option value="{{$item->id}}" {{$item->id == $data[0]->class_id ? 'selected': ''}}>{{$item->name}}</option>
                  @endforeach
                </select>
                @error('class_id')
             ...
             <div class="row">
              ...
              <div class="form-group col-lg-4">
                ...
                <select id="inputState5" name="subject_id[]" class="form-control @error ('subject_id') is-invalid @enderror">
                  ...
                  <option value="{{$item->id}}" {{$item->id ==  $singleData->subject_id ? 'selected': ''}}>{{$item->name}}</option>
                  @endforeach
                </select>
                @error('subject_id')
                  <span class="text-danger">{{$message}}</span>
              ...
              <div class="form-group col-lg-2">
                ...
                <input type="text" name="full_mark[]" value="{{old('full_mark', $singleData->full_mark)}}" class="form-control  @error ('full_mark') is-invalid @enderror" placeholder="full mark">
                @error('full_mark')
                    <span class="text-danger">{{$message}}</span>
                @enderror
              </div>
              ...
              <div class="form-group col-lg-2">
                ...
                <input type="text" name="pass_mark[]" value="{{old('pass_mark', $singleData->pass_mark)}}" class="form-control  @error ('pass_mark') is-invalid @enderror" placeholder="pass mark">
                @error('pass_mark')
                    <span class="text-danger">{{$message}}</span>
                @enderror
              </div>
              ...
              <div class="form-group col-lg-2">
                ...
                <input type="text" name="subjective_mark[]" value="{{old('subjective_mark', $singleData->subjective_mark)}}" class="form-control  @error ('subjective_mark') is-invalid @enderror" placeholder="subjective mark">
            ...
            <button type="submit" class="btn mb-2 btn-secondary">Update Change</button>
          </form>
